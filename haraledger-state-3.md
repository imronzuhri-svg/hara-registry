...
**Stress test 2026-06-01 — VALIDATED on the dedicated host (after a false start).** First two runs (527/648 TPS, "800/800 confirmed") were INVALID — every Phase C tx reverted (status=0x0, ~121k gas) because the deployer key in hand (`0xc9064736…b408bf5`) lacked `MINTER_ROLE`. Fixed by granting MINTER_ROLE to it via the admin key (read from Vault using the root token in `ops/secrets.txt`), then **revoking after**. The valid re-run: **200×500 = 99,800 transfers, 400/400 status=0x1, avg 6.88M gas/tx (2.75B total), 276.4s, 361 TPS**; executeChain receipts show 500 logs each; batches now visible in the custody viewer + Blockscout. Chunks 4–20s (heavy ones hit the 8s build cap) but clean — no reverts/flip-flops. **Import-CPU wall confirmed cleared under real load.** Lesson: verify receipt status+gas+logs before reporting TPS. SECURITY: `ops/secrets.txt` holds the Vault root token + GitHub PAT in plaintext (now gitignored) — rotate both + move to password manager. See [[stress-test-results]].
