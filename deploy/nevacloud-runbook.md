...
docker run --rm --network hara-platform \
  -v /opt/hara-ledger/deploy/chain/genesis:/work:ro \
...
scp hara@hara-stateful:/opt/hara-ledger/deploy/chain/genesis/genesis.json \
...
scp hara@hara-stateful:/opt/hara-ledger/deploy/data/.env      deploy/data/.env
...
ssh hara@hara-stateless "
  echo 'PQ_ANCHOR_REGISTRY_ADDRESS=$ADDR' >> /opt/hara-ledger/deploy/services/.env
  cd /opt/hara-ledger
  docker compose -f deploy/services/docker-compose.yml \
...
| Besu boots and crash-loops with `Unable to load genesis file ... /opt/besu/genesis/genesis.json` | `deploy/chain/genesis/` is empty on this VPS (init ran on hara-stateful only) | `scp hara@hara-stateful:/opt/hara-ledger/deploy/chain/genesis/genesis.json deploy/chain/genesis/` then restart |
